# -*- coding: utf-8 -*-
# ==============================================================================
# Liver Cancer Classification Training Script (for Local Execution with GPU)
# ==============================================================================
...
# --- 경로 설정 ---
DRIVE_BASE_PATH = 'C:/Users/21/Desktop/간암' # <<<--- 사용자의 실제 로컬 데이터 폴더 경로로 반드시 수정하세요!
BASE_PATH = './'
...
# --- 작업 경로 ---
WORK_DIR = os.path.join(BASE_PATH, 'Liver_classification_project') # Updated project name slightly
EXTRACT_BASE_PATH = os.path.join(WORK_DIR, 'data')
CANCER_EXTRACT_PATH = os.path.join(EXTRACT_BASE_PATH, 'Task03_Liver_extracted') # Renamed
NORMAL_EXTRACT_PATH = os.path.join(EXTRACT_BASE_PATH, 'normal_Liver_extracted') # Renamed
CANCER_EXPECTED_SUBFOLDER = "Task03_Liver"
NORMAL_EXPECTED_SUBFOLDER = "normal_Liver"
...
BATCH_SIZE = 4
LEARNING_RATE = 1e-4
# *** 변경: Train/Validation/Test 분할 비율 조정 ***
TEST_SPLIT = 0.15       # 전체 데이터 중 테스트 세트 비율 (15% 유지)
VALIDATION_SPLIT = 0.25 # *나머지* 데이터 중 검증 세트 비율 증가 (예: 15% -> 25%)
# 이렇게 하면 Test 15%, Validation은 나머지 85% 중 25% (전체의 약 21%), Train은 약 64%가 됩니다.
# ------------------------------------------
RANDOM_STATE = 42
BALANCE_DATA = True
...
def prepare_classification_files(cancer_extract_root, normal_extract_root,
                                 ...
                                 image_folder="imagesTr", # 기본 이미지 폴더 이름
                                 balance=True,
    ...
    # 정상 데이터 (Class 0)
    normal_image_subfolder_path = os.path.join(normal_expected_subfolder, image_folder)
    print(f"  Adjusted normal image subfolder path relative to adjusted root: {normal_image_subfolder_path}") # 디버깅용 출력 추가

    normal_files, processed_counts['normal'], skipped_counts['normal'] = find_images_with_label(
        normal_root_adj, # 이미 첫 번째 normal_Liver 폴더까지 조정된 경로
        normal_image_subfolder_path, # 그 하위의 'normal_Liver/imagesTr' 경로 전달
        0
    )
...
# ==============================================================================
from monai.transforms import  RandAffined, RandAdjustContrastd, RandScaleIntensityd
...
train_transforms = Compose(
    [
        ...
        # *** 수정: 'd' 추가 ***
        RandAffined( # 회전, 스케일, 이동 변환을 종합적으로 적용 ('d' 추가)
            keys=[IMAGE_KEY],
            mode="bilinear",
            prob=0.7,
        ...
        RandAdjustContrastd(keys=[IMAGE_KEY], prob=0.3, gamma=(0.7, 1.5)), # ('d' 추가)
        RandScaleIntensityd(keys=[IMAGE_KEY], prob=0.3, factors=0.2),     # ('d' 추가)

        RandShiftIntensityd(keys=[IMAGE_KEY], offsets=0.1, prob=0.5),
        RandGaussianNoised(keys=[IMAGE_KEY], prob=0.2, mean=0.0, std=0.05),
...
# 검증/테스트 시에는 Augmentation을 적용하지 않으므로 val_transforms는 수정할 필요 없음
...
if __name__ == '__main__':
    ...
    print("="*60)
    print(" Liver Cancer Classification Training Script (Local GPU)")
    print("="*60)
